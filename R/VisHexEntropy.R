...
VisHexEntropy <- function(object, group.by = 'seurat_clusters', reduction = 'umap', bins = 30) {
  ...
  if (!is.numeric(bins) || bins < 2) stop('bins must be an integer >= 2')
  xr <- range(emb$X1, na.rm = TRUE); yr <- range(emb$X2, na.rm = TRUE)
  if (!is.finite(diff(xr)) || diff(xr) == 0) xr <- scales::expand_range(xr, mul = 0.01)
  if (!is.finite(diff(yr)) || diff(yr) == 0) yr <- scales::expand_range(yr, mul = 0.01)
  bx_breaks <- seq(xr[1], xr[2], length.out = bins + 1)
  by_breaks <- seq(yr[1], yr[2], length.out = bins + 1)
  emb$bx <- cut(emb$X1, breaks = bx_breaks, include.lowest = TRUE, right = TRUE)
  emb$by <- cut(emb$X2, breaks = by_breaks, include.lowest = TRUE, right = TRUE)
  agg <- emb |> dplyr::count(bx, by, group) |> tidyr::pivot_wider(names_from = group, values_from = n, values_fill = 0)
  ...
  ggplot2::ggplot(df, ggplot2::aes(x = x, y = y, fill = entropy)) + ggplot2::geom_tile() + ggplot2::scale_fill_viridis_c() +
    svpp_theme() + ggplot2::labs(x = paste0(toupper(reduction),'1'), y = paste0(toupper(reduction),'2'), fill = 'Entropy')
  }
